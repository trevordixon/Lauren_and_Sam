...
if (!empty($_POST)) {
  ...
  if (!empty($_POST['rowid'])) {
    ...
    // Insert new
    $_POST['Requested Invite'] = date('Y-M-d H:i:s');
    echo $ftclient->query(SQLBuilder::insert($table_id, $_POST));
  }
} elseif (array_key_exists('email', $_GET)) {
  $result = explode("\n", $ftclient->query(SQLBuilder::select($table_id, array('rowid', 'Name', 'Address', 'City', 'State', 'Zip', 'Phone', 'Notes'), "'Email'='" . $_GET['email'] . "'")));
